    ...
    return (
        <section className="w-full max-w-2xl mx-auto p-6">
            <div className="flex items-center gap-2 mb-6">
                <span className="text-2xl font-extrabold text-blue-700">{t('title')}</span>
                <span className="bg-blue-100 text-blue-600 text-xs font-semibold px-2 py-0.5 rounded-full">{t('live')}</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
                {indicators.map((item) => (
                    <div
                        key={item.label}
                        className={`relative bg-gradient-to-br ${item.color} rounded-xl shadow-md p-6 flex flex-col items-center border border-gray-100 transition-transform hover:scale-105 hover:shadow-lg h-[180px]`}
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
                {extraStats.map((stat) => (
                    ...
                    >
                        ...
                        <span className={`font-bold text-lg ${stat.color} flex-shrink-0`}>{stat.value}</span>
